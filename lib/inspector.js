...
const schemaInspection = (oldSchemaFile, newSchema) => {
  const newSchemaString = printSchema(newSchema);
  try {
    logger.info(info, 'Schema inspection.');
    const changes = [];
    if (fs.existsSync(oldSchemaFile)) {
      changes.push(...getChangesInSchema(oldSchemaFile, newSchemaString));
    ...
    logChanges(changes);
    resolveInspection(changes, oldSchemaFile, newSchemaString);
  } catch (err) {
    logger.error(err);
    process.exit(status.ERROR_EXIT_STATUS);
  }
